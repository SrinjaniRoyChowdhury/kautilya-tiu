-- Allotments are free-text only. Do not resolve SLR / canonical names from portfolio_config.
...
-- Clear stored portfolio matrices; capacity is the source of truth for seats.
update public.committees
set portfolio_config = '[]'::jsonb
where coalesce(jsonb_typeof(portfolio_config), 'null') <> 'null'
  and portfolio_config <> '[]'::jsonb;
