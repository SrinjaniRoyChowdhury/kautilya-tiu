...
export async function committeeAllocationRows(committeeId: string) {
  ...
  const { data: committee } = await supabase
    .from("committees")
    .select("id, short_name, name, edition_id")
    .eq("id", committeeId)
    .maybeSingle();
  if (!committee) return null;
  const { data } = await supabase
    .from("registrations")
    .select("allocated_slr, allocated_portfolio, status, users:user_id (full_name, email)")
    .eq("committee_id", committeeId)
    .is("deleted_at", null)
    .neq("status", "CANCELLED")
    .not("allocated_portfolio", "is", null)
    .order("allocated_portfolio", { ascending: true });
  type Row = {
  ...
  const headers = ["Portfolio", "SLR No.", "Delegate name", "Delegate email", "Status"];
  const rows = ((data as Row[] | null) ?? []).map((row) => {
    const user = Array.isArray(row.users) ? row.users[0] : row.users;
    return [
