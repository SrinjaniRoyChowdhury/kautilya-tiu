...
import { useRef, useEffect, type CSSProperties, type ReactNode, type MouseEventHandler, type RefObject } from 'react';
...
const SpecularButton = ({
  ...
  useEffect(() => {
    ...
    const update = (now: number) => {
      ...
      program.uniforms.uBaseWidth.value = p.thickness * dpr * 1.35;
  ...
  const surfaceClass = `relative m-0 inline-flex cursor-pointer items-center justify-center border-none font-medium leading-none tracking-[0.01em] outline-none transition-transform duration-150 active:scale-[0.97] disabled:cursor-default disabled:opacity-55 disabled:active:scale-100 [color:var(--sb-text-color)] [border-radius:var(--sb-radius)] [background:color-mix(in_srgb,var(--sb-tint)_calc(var(--sb-tint-opacity)*100%),transparent)] [backdrop-filter:blur(var(--sb-blur))] shadow-[inset_0_1px_0_rgba(255,255,255,0.04),0_8px_24px_rgba(0,0,0,0.25)] focus-visible:outline-2 focus-visible:outline-offset-[3px] ${SIZES[size] || SIZES.md}${className ? ` ${className}` : ''}${disabled ? ' pointer-events-none opacity-55' : ''}`;
  const surfaceStyle = {
    '--sb-radius': `${radius}px`,
    '--sb-tint': tint,
